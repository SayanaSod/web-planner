...
class TasksRepository(Protocol):
    ...
    async def list(self, user_id: str, *, date_eq: Optional[date] = None, type_eq: Optional[str] = None, q: Optional[str] = None) -> list: ...
    ...
    async def insert_many_nager(self, user_id: str, items: list) -> tuple: ...
    async def insert_many_generic(self, user_id: str, items: list) -> tuple: ...
...
class MotorTasksRepository(TasksRepository):
    ...
    async def list(self, user_id: str, *, date_eq: Optional[date] = None, type_eq: Optional[str] = None, q: Optional[str] = None) -> list:
        ...
        return [self._to_public(d) async for d in cursor]
    ...
    async def insert_many_nager(self, user_id: str, items: list) -> tuple:
        return await self.insert_many_generic(user_id, items)
    ...
    async def insert_many_generic(self, user_id: str, items: list) -> tuple:
        ...
        for it in items:
            ...
            source_id = it.get("meta", {}).get("source_id")
            if not source_id:
                continue
            res = await self.coll.update_one(
                {"user_id": it["user_id"], "meta.source_id": source_id},
                {"$setOnInsert": it},
                upsert=True,
            )
            if res.upserted_id:
...
class InMemoryUsersRepository(UsersRepository):
    ...
    async def create(self, email: str, password_hash: str) -> UserDict:
        ...
        uid = uuid.uuid4().hex
        doc = {"id": uid, "email": email, "password_hash": password_hash}
        self._by_id[uid] = doc
